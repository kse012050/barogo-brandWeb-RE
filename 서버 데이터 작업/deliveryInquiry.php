<?php include "head.php"; ?>
<body>
    <div class="inquiryPage">
        <header>
            <div class="CW">
                <h1><a href="index.php"><img src="images/common/logo.png" alt="barogo logo"></a></h1>
                <a href="index.php">뒤로가기</a>
                <p>배달대행 문의</p>
            </div>
        </header>

        <section class="CW inquiryArea">
            <div class="textArea">
                <h2 class="TC-01">배달대행 문의</h2>
                <p>
                    원하시는 배달대행 서비스 단위를<br/>
                    선택해 주세요
                </p>
            </div>
            <ul class="overLine-right guideArea">
                <li>
                    <a href="deliveryInquiry-general.php">
                        일반 매장<br/>
                        배달대행 문의
                    </a>
                </li>
                <li>
                    <a href="deliveryInquiry-enterprise.php" data-click="popup01">
                        기업 본사<br/>
                        배달제휴 문의
                    </a>
                    <div class="popupBox" data-popup="popup01">
                        <div>
                            <p>
                                프랜차이즈 소속의 <strong>개별 매장에서 문의하시는 경우,</strong><br/>
                                <mark>[단일 매장 배달대행 문의]</mark>를 이용해주세요!<br/>
                                보다 정확하고 빠른 연결이 가능합니다.
                            </p>
                            <p>
                                <strong>프렌차이즈/기업 본사</strong>에서<br/>
                                바로고와의 서비스 제휴를 원하시는게 맞나요?
                            </p>
                            <a href="deliveryInquiry-enterprise.php">네, 기업본사 문의입니다</a>
                            <button>아니요, 다시할게요</button>
                        </div>
                    </div>
                </li>
            </ul>
        </section>
    </div>
</body>
</html>